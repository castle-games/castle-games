...
public:
  ...
  static CaseInsensitiveMultimap parse(const std::string &query_string) noexcept {
    ...
    if (name_pos < query_string.size()) {
      ...
      if (!name.empty()) {
        auto value =
            value_pos >= query_string.size() ? std::string() : query_string.substr(value_pos);
        result.emplace(std::move(name), Percent::decode(value));
      }
    }
...
public:
  ...
  public:
    ...
    public:
      /// Parse Set-Cookie or Content-Disposition header field value. Attribute values are
      /// percent-decoded.
...
public:
  ...
  static bool parse(std::istream &stream, std::string &method, std::string &path,
                    std::string &query_string, std::string &version,
