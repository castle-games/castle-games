...
public:
  ...
  /// Convenience function to perform synchronous request. The io_service is run within this
  /// function. If reusing the io_service for other tasks, use the asynchronous request functions
  /// instead. Do not use concurrently with the asynchronous request functions.
  std::shared_ptr<Response>
  request(const std::string &method, const std::string &path = std::string("/"),
          string_view content = "",
  ...
  /// instead. Do not use concurrently with the asynchronous request functions.
  std::shared_ptr<Response>
  request(const std::string &method, const std::string &path, std::istream &content,
          const CaseInsensitiveMultimap &header = CaseInsensitiveMultimap()) {
    std::shared_ptr<Response> response;
    error_code ec;
    request(method, path, content, header,
  ...
  void
  request(const std::string &method, const std::string &path, string_view content,
...
protected:
  ...
  void connect(const std::shared_ptr<Session> &session) override {
    if (!session->connection->socket->lowest_layer().is_open()) {
      ...
      resolver->async_resolve(*query, [this, session,
