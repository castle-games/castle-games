...
public:
  class Response : public std::enable_shared_from_this<Response>, public std::ostream {
    ...
    std::list<std::pair<std::shared_ptr<asio::streambuf>, std::function<void(const error_code &)>>>
        send_queue;

    Response(std::shared_ptr<Session> session_, long timeout_content) noexcept
        : std::ostream(nullptr), session(std::move(session_)), timeout_content(timeout_content),
    ...
    void send_from_queue() {
      ...
      strand.post([self]() {
        asio::async_write(
            *self->session->connection->socket, *self->send_queue.begin()->first,
    ...
    void
    send_on_delete(const std::function<void(const error_code &)> &callback = nullptr) noexcept {
      session->connection->set_timeout(timeout_content);
      auto self = this->shared_from_this(); // Keep Response instance alive through the following
                                            // async_write
  ...
  public:
    ...
    /// Use this function if you need to recursively send parts of a longer message, or when using
    ...
    void write(string_view content,
               const CaseInsensitiveMultimap &header = CaseInsensitiveMultimap()) {
  ...
  public:
    ...
    unsigned short remote_endpoint_port() const noexcept { return remote_endpoint->port(); }
...
public:
  ...
  public:
    /// Port number to use. Defaults to 80 for HTTP and 443 for HTTPS. Set to 0 get an assigned
    /// port.
    unsigned short port;
    ...
    std::string address;
    /// Set to false to avoid binding the socket to an address that is already in use. Defaults to
    /// true.
...
public:
  ...
  unsigned short bind() {
    ...
    if (config.fast_open) {
      ...
      error_code ec;
      acceptor->set_option(asio::detail::socket_option::integer<IPPROTO_TCP, TCP_FASTOPEN>(qlen),
                           ec);
#endif // End Linux
    }
  ...
  /// If you know the server port in advance, use start() instead.
  /// Accept requests, and if io_service was not set before calling bind(), run the internal
  /// io_service instead. Call after bind().
  void accept_and_run() {
    acceptor->listen();
    accept();
    ...
    if (internal_io_service) {
      ...
      // If thread_pool_size>1, start m_io_service.run() in (thread_pool_size-1) threads for
      // thread-pooling
      threads.clear();
...
protected:
  ...
  std::shared_ptr<Connection> create_connection(Args &&... args) noexcept {
    ...
    auto connection =
        std::shared_ptr<Connection>(new Connection(handler_runner, std::forward<Args>(args)...),
    ...
    asio::async_read_until(
        *session->connection->socket, session->request->streambuf, "\r\n\r\n",
        [this, session](const error_code &ec, std::size_t bytes_transferred) {
          ...
          if (!ec) {
            // request->streambuf.size() is not necessarily the same as bytes_transferred, from
            // Boost-docs: "After a successful async_read_until operation, the streambuf may contain
            // additional data beyond the delimiter" The chosen solution is to extract lines from
            // the stream directly when parsing the header. What is left of the streambuf (maybe
            // some bytes of the content) is appended to in the async_read-function below (for
            // retrieving content).
            ...
            if (header_it != session->request->header.end()) {
              ...
              auto chunks_streambuf =
                  std::make_shared<asio::streambuf>(this->config.max_request_streambuf_size);
  ...
  void write(const std::shared_ptr<Session> &session,
    ...
    session->connection->set_timeout(config.timeout_content);
    auto response = std::shared_ptr<Response>(
        new Response(session, config.timeout_content), [this](Response *response_ptr) {
